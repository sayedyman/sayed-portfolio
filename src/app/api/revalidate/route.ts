...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const slug = body?.slug?.current || (typeof body?.slug === 'string' ? body.slug : undefined)

    console.log('[Webhook] Payload received:', JSON.stringify(body))
    console.log(`[Webhook] Detected type: ${type}, slug: ${slug}`)

    if (type === 'project' || type === 'projectType' || type === 'category') {
